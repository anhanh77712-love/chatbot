...
export default function Home() {
  ...
  const handleAddOrEdit = async () => {
    ...
    if (editingId) {
      await fetch('/api/products', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: editingId, name: newName, link: newLink }) });
      setEditingId(null);
    } else {
      ...
      <div style={{ flex: 1, padding: '40px', overflowY: 'auto' }}>
        ...
        {activeTab === 'products' && (
          <div style={cardStyle}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead><tr style={{ background: '#ffb6c1', color: '#fff' }}><th style={thStyle}>Tên Áo</th><th style={thStyle}>Đường Link</th><th style={thStyle}>Hành Động</th></tr></thead>
              <tbody>
                {products.map(p => (
        ...
        {activeTab === 'aivip' && (
          <div style={{...cardStyle, maxWidth: '800px', margin: '0 auto', border: '2px solid #ff69b4'}}>
            ...
            <p style={{textAlign: 'center', marginBottom: '20px', color: '#ff69b4'}}>Bạn chỉ cần nhập tình huống, AI sẽ tự quét kho đồ và chọn áo phù hợp nhất!</p>
            ...
            {vipResponse && (
              <div style={{...responseBoxStyle, background: '#fff', border: '3px solid #ffb6c1'}}>
                <button onClick={handleVipCopy} title="Sao chép" style={copyBtnStyle}>{isVipCopied ? '✅' : '📋'}</button>
                {vipResponse}
              </div>
            )}
          </div>
